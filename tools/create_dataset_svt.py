...
def main(output_root, checkValid=True):

    root_path = "/root/svt1/"
    alphabet = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"

    output_path = output_root + "lmdb_svt_test"  # 这里改是 test 还是 train
    env = lmdb.open(output_path, map_size=1099511627776)

    lex_path = output_root + "svt_lex.txt"
    lex_f = open(lex_path, "w")

    cache = {}
    cnt = 1

    doc = minidom.parse(root_path + "test.xml")
    ...
    for img_tag in img_tags:
        ...
        lex = img_tag.getElementsByTagName("lex")[0].firstChild.data
        lex_f.write(path_key + ' ' + lex + '\n')
    ...
    print('Created %s with %d samples' % (output_path, nSamples))


if __name__ == '__main__':
    main("/root/project/data/")
