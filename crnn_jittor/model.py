...
class CRNN(jt.Module):
    ...
    def execute(self, x):  # (256, 1, 32, 100)
        ...
        T, b, h = x.shape
        x, (h_n, c_n) = self.rnn(x, (jt.zeros((4, b, 256)), jt.zeros((4, b, 256))))  # (24, 256, 1024) ---- h_0, c_0 default to zero if not provided; output dim double because rnn is bidirectional
        T, b, h = x.shape
